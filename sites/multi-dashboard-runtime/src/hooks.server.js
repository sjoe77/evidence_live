...
export function handle({ event, resolve }) {
	...
	if (dev) {
		const authHeaders = {
			'X-ID-Token': event.request.headers.get('X-ID-Token'),
			'X-Email': event.request.headers.get('X-Email'),
			'X-Client-ID': event.request.headers.get('X-Client-ID')
		};
		if (authHeaders['X-ID-Token']) {
			log.debug('Evidence received auth headers', authHeaders);
		}
	}
